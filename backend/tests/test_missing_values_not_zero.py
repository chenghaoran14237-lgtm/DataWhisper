...
@pytest.mark.asyncio
async def test_chart_keeps_missing_as_none(tmp_path):
    os.environ["ENV"] = "test"
    ...
    df = pd.DataFrame({"month": ["Jan", "Feb"], "sales": [10, None]})
    bio = BytesIO()
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        yvals = spec["series"][0]["values"]
        assert yvals[1] is None  # ✅ Feb 缺失应该是 None，而不是 0
